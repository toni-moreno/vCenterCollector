...
import { VCenterServerService } from './vcenterserver.service';
...
import { VCenterServerComponentConfig, TableRole, OverrideRoleActions } from './vcenterserver.data';
import { InfluxServerService } from '../influxserver/influxservercfg.service';

declare var _:any;

@Component({
  selector: 'vcenterserver-component',
  providers: [VCenterServerService, BlockUIService, ValidationService,InfluxServerService],
  templateUrl: './vcenterserver.component.html',
  styleUrls: ['../../css/component-styles.css']
})

export class VCenterServerComponent implements OnInit {
  ...
  public defaultConfig : any = VCenterServerComponentConfig;
  ...
  constructor(public _blocker: BlockUIService, public influxServerService: InfluxServerService, public vcenterserverService: VCenterServerService, public exportServiceCfg : ExportServiceCfg, builder: FormBuilder) {
  ...
  createStaticForm() {
    this.sampleComponentForm = this.builder.group({
      ...
      User: [this.sampleComponentForm ? this.sampleComponentForm.value.User : 'vcenteruser', Validators.required],
      ...
      VCenterAPIDebug: [this.sampleComponentForm ? this.sampleComponentForm.value.VCenterAPIDebug : 'false', Validators.required],
  ...
  testVCenterServerConnection(data:any) {
    this.vcenterserverService.testVCenterServer(data,true)
    .subscribe(
    data =>  {
  ...
  importVCenterDevices(data:any) {
    ...
    if (r == true) {
      ...
      this.vcenterserverService.importVCenterDevices(data, true)
        .subscribe(
        data =>  {
          this.alertHandler = {msg: 'HCM Version: '+data['Message'], result : data['Result'], elapsed: data['Elapsed'], type: 'success', closable: true}
          this._blocker.stop();
  ...
  this.vcenterserverService.getVCenterServerItem(null)
      .subscribe(
      data => {
    ...
    switch (action.option) {
      ...
      case 'editenabled':
        this.enableEdit();
      break;
      case 'importVCenterDevices':
        this.importVCenterDevices(action.event);
      break;
    }
  }
  ...
  removeItem(row) {
    ...
    this.vcenterserverService.checkOnDeleteVCenterServerItem(id)
      .subscribe(
        data => {
  ...
  editSampleItem(row) {
    let id = row.ID;
    this.getInfluxServersforDevices()
    this.vcenterserverService.getVCenterServerItemById(id)
      .subscribe(data => {
  ...
  deleteSampleItem(id, recursive?) {
    if (!recursive) {
    this.vcenterserverService.deleteVCenterServerItem(id)
      .subscribe(data => { },
      err => console.error(err),
      () => { this.viewModalDelete.hide(); this.reloadData() }
      );
    } else {
      return this.vcenterserverService.deleteVCenterServerItem(id,true)
      .do(
  ...
  saveSampleItem() {
    if (this.sampleComponentForm.valid) {
      this.vcenterserverService.addVCenterServerItem(this.sampleComponentForm.value)
    ...
    if(!recursive) {
      if (this.sampleComponentForm.valid) {
        ...
        if (this.sampleComponentForm.value.ID != this.oldID) {
          r = confirm("Changing VCenterServer Instance ID from " + this.oldID + " to " + this.sampleComponentForm.value.ID + ". Proceed?");
        }
        if (r == true) {
          this.vcenterserverService.editVCenterServerItem(this.sampleComponentForm.value, this.oldID)
            .subscribe(data => { console.log(data) },
            err => console.error(err),
            () => { this.editmode = "list"; this.reloadData() }
            );
      ...
      return this.vcenterserverService.editVCenterServerItem(component, component.ID, true)
      .do(
