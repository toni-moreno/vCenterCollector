...
import (
	...
	"github.com/toni-moreno/vCenterCollector/pkg/data/impexp"
	"github.com/toni-moreno/vCenterCollector/pkg/webui"
)
...
var (
	...
	dataDir    = confDir
	configFile = filepath.Join(confDir, "vcentercollector.toml")
)
...
func init() {
	...
	if _, err := os.Stat(configFile); err == nil {
		...
		viper.AddConfigPath("/etc/vcentercollector/")
		viper.AddConfigPath("/opt/vcentercollector/conf/")
	...
	if len(cfg.General.LogDir) > 0 {
		...
		//Log output
		f, _ := os.OpenFile(logDir+"/vcentercollector.log", os.O_APPEND|os.O_CREATE|os.O_RDWR, 0644)
		log.Out = f
	}
	...
	vcenter.SetDBConfig(&agent.DBConfig, &agent.MainConfig.Database)
	vcenter.SetLogDir(logDir)
