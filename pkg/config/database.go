...
func (dbc *DatabaseCfg) InitDB() {
	...
	if err = dbc.x.Sync(new(VCenterCfg)); err != nil {
		log.Fatalf("Fail to sync database VCenterConfig: %v\n", err)
	}
...
func (dbc *DatabaseCfg) LoadDbConfig(cfg *DBConfig) {
	...
	//Load VCenter engines map
	cfg.VCenter, err = dbc.GetVCenterCfgMap("")
	if err != nil {
		log.Warningf("Some errors on get VCenter servers  URL :%v", err)
	}
