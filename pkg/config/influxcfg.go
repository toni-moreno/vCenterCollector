...
func (dbc *DatabaseCfg) DelInfluxCfg(id string) (int64, error) {
	...
	affecteddev, err = session.Where("outdb='" + id + "'").Cols("outdb").Update(&VCenterCfg{})
	if err != nil {
		session.Rollback()
		return 0, fmt.Errorf("Error on Delete Device with id on delete VCenterCfg with id: %s, error: %s", id, err)
	}
...
func (dbc *DatabaseCfg) UpdateInfluxCfg(id string, dev InfluxCfg) (int64, error) {
	...
	if id != dev.ID { //ID has been changed
		affecteddev, err = session.Where("outdb='" + id + "'").Cols("outdb").Update(&VCenterCfg{OutDB: dev.ID})
		if err != nil {
			session.Rollback()
...
func (dbc *DatabaseCfg) GetInfluxCfgAffectOnDel(id string) ([]*DbObjAction, error) {
	...
	for _, val := range devices {
		obj = append(obj, &DbObjAction{
			...
			ObID:     val.ID,
			Action:   "Reset InfluxDB Server from VCenter  Device to 'default' InfluxDB Server",
		})
