...
var (
	// Bus the bus messaging system to send messages over the servers
	Bus = bus.NewBus()
...
func PrepareInfluxDBs() map[string]*output.InfluxDB {
	...
	if defFound == false {
		//no devices configured  as default device we need to set some device as itcan send data transparent to devices goroutines
...
// DeviceProcessStart start all server goroutines
func DeviceProcessStart() {
	mutex.RLock()

	for s, c := range dataDevices {
		log.Infof("Starting Server device %s", s)
...
func LoadConf() {
	...
	//Init  devices
	for k, c := range DBConfig.VCenter {
		//Inticialize each  device and
		dev := vcenter.New(c)
...
func ReloadConf() (time.Duration, error) {
	...
	log.Infof("RELOADCONF INIT: begin Gather processes stop... at %s", start.String())
